...
void do_interrupt(CPUARMState *env)
{
    ...
    if (env->rabbits.sw_single_step > 0)
        env->rabbits.sw_single_step = -env->rabbits.sw_single_step;
    #endif
    ...
    case EXCP_UDEF:
        ...
        if(cpu_single_env->rabbits.tr_id != NULL){
            tr_commit(0, 0); // TODO: Classify this instruction as aborted/exception
        }
#endif
        break;
    case EXCP_SWI:
...
void HELPER(invalidate_dcache_ALL)(CPUState *env)
{
    dcache_invalidate_all();
}
void HELPER(flush_dcache_ALL)(CPUState *env)
{
    dcache_flush_all();
}

void HELPER(invalidate_dcache_MVA)(CPUState *env, uint32_t val)
{
    uint32_t phys_addr;
    target_ulong page_size;
    int prot;
    ...
    is_user = 0; // TODO: Uses the IS_USER instead
    access_type = 0; // TODO: I don't know exactly what it means in this case

    ret = get_phys_addr(env, val, access_type, is_user,
                        &phys_addr, &prot, &page_size);
    if (ret == 0){
        dcache_invalidate(phys_addr);
    } else {
        printf("error getting the physical value = 0x%08x\n", val);
        dcache_invalidate(val);
    }    
}

void HELPER(flush_dcache_MVA)(CPUState *env, uint32_t val)
{
	 dcache_flush(val); // TODO: Must be updated to take into account the MMU as done to invalidation
}
...
unsigned long get_phys_addr_gdb (unsigned long addr)
{
    ...
    if (!get_phys_addr (cpu_single_env, addr, 0, 0, &phys_ptr,
        &prot, &page_size))
        addr = phys_ptr;
