...
{
    GtkWidget       *w = drawing_area[(long)data];
...
long                crt_tooltip_drawing_area = -1;
...
void UpdateDrawingAreaTooltip (bool redraw)
{
    char                    s[100];
    long long               tm;
    long                    idx = crt_tooltip_drawing_area;
...
{
    crt_tooltip_drawing_area = (long) data;
...
int main (int argc, char **argv)
{
    ...
    for (i = 0; i < ngrf; i++)
    {
        ...
        g_signal_connect (G_OBJECT (event_box), "button_press_event", G_CALLBACK (label_press_event), (gpointer) i);
