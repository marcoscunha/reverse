...
main(int argc, char *argv[])
{
    ...
    /* Wait forever to be awaken to refresh the screen */
    do {
       int rv;
       char c;
       fd_set rfds;
       /* One day should do, even if we decode very very slowly */
       struct timeval tv = {60*24, 0};

       FD_ZERO(&rfds);
       FD_SET(0, &rfds);
       rv = select(1, &rfds, NULL, NULL, &tv);
       if (rv == -1)  /* Select has an issue, let's die */
          goto SDL_error;
       else if (rv) { /* Let's refresh */
          if (read(0, &c,  sizeof(c)) != sizeof(c)) {
             EPRINTF("read after select failed!\n");
             exit(EXIT_FAILURE);
          }
          DPRINTF("Refresh\n");
          ...
          convert_and_display();
          DPRINTF("Refresh -- END\n");
       } else {       /* Not real-time then */
          EPRINTF("timeout on select!\n");
          exit(EXIT_FAILURE);
       }
    } while (1);
