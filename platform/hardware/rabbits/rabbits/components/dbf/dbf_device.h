...
enum dbf_block_master_status {
    DBF_MASTER_READY        = 0,
    DBF_MASTER_CMD_SUCCESS  = 1,
    DBF_MASTER_CMD_ERROR    = 2,
};

class dbf_device_master : public master_device
{
    public:
        dbf_device_master (const char *_name);
        ~dbf_device_master();
...
public:
    ...
    dbf_device (sc_module_name module_name);
    ~dbf_device ();
