...
void tg_device::write (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
    switch (ofs)
    {
    case 0x480:
...
void tg_device::read (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
    int             i;
    ...
    *((uint32_t *)data + 1) = 0;
    ...
    case 0x80:
        if (!tg_bytes_left)
            reset_input ();
        *((uint32_t *)data + 0) = (tg_bytes_left + 3) / 4;
        break;
    default:
...
void tg_device::rcv_rqst (uint32_t ofs, uint8_t be,
