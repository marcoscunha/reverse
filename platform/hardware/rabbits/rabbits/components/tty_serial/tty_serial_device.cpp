...
#define DPRINTF(fmt, args...)                                           \
    do { fprintf(stderr, "DEBUG tty_serial_device (%s): " fmt , name(), ##args); } while (0)
...
tty_serial_device::tty_serial_device (sc_module_name _name) : slave_device (_name)
{
    ...
    if (!(s_pid_tty[s_nb_tty++] = fork ()))
    {
        ...
        if (execlp ("xterm",
                    ...
                    "-sb","-sl","1000",
                    "-l", "-lf", logfile,
                    "-n", (const char *)_name, "-T", (const char *)_name,
 ...
 * @param bErr
 */
void tty_serial_device::write (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
    uint8_t                     ch;
    uint32_t                    value;
    ...
    if (ofs != 0)
#endif
    DPRINTF ("%s to 0x%lx - value 0x%lx\n", __FUNCTION__, (unsigned long) ofs, (unsigned long) value);
 ...
 * @param bErr
 */
void tty_serial_device::read (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
    uint32_t            c, *pdata;
 ...
 * @param bWrite
 */
void tty_serial_device::rcv_rqst (uint32_t ofs, uint8_t be,
