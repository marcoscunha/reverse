...
app_loader::load_binary_image(const char *file, uint32_t mast_id, uint32_t load){
    ...
    #ifdef TRACE_EVENT_ENABLED
    {
        ...
        fd_init = fopen("mem_init", "r+");
        fseek(fd_init, 0, SEEK_SET);

        fseek(fd_init, 0, SEEK_SET);
        ret = fscanf(fd_init, "N_SECTIONS=%08d\n",&sections);
        cout << "ret " << ret <<  " " << EOF << endl;
        if (ret == EOF ){
           sections = 1;
        }else{
           sections++;
        ...
        fseek(fd_init, 0, SEEK_SET);
        sprintf(str, "N_SECTIONS=%08d\n", sections);
        fwrite(str, strlen(str), 1, fd_init);

        fseek(fd_init,0, SEEK_END);
        sprintf(str, "SECTION=%08d\n", sections);
        sprintf(str_tmp, "SIZE=%08x\n", size);
        str = strcat(str, str_tmp);
        sprintf(str_tmp, "OFFSET=%08x\n", ofs);
        str = strcat(str, str_tmp);
               
        fwrite(str, strlen(str), 1, fd_init);
        fwrite(mem + ofs, size, 1, fd_init);

        cout << "sections " << hex<< sections << "\nsize "  << size << "\nofs " << ofs << endl;
...
app_loader::load_linux_image(linux_args_t *args, uint32_t mast_id)
{
    ...
    FILE* fp = fopen("mem_init","r");

    cout << fp << endl;
    if(fp != NULL){
        fclose(fp);
        printf("Mem init problem: remove mem_init file before start\n");
        exit(1);
    }else{
        FILE* fp = fopen("mem_init","w+");
        fclose(fp);
    }
