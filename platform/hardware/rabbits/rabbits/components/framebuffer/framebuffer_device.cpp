...
int     fb_device::pids_ramdac[128];
uint8_t *fb_device::shm_addr[128][2];
int     fb_device::shmids[128][2];
int     fb_device::nb_fb = 0;
...
/**
*  Name: fb_device::fb_device
*  Function: frame buffer device constructor
*  name: device name
*  master_id: master device id connected to this component
*  reset_status:
*/
fb_device::fb_device (sc_module_name _name, fb_reset_t *reset_status)
    :sc_module(_name)
{
    ...
    slave = new fb_device_slave("sl", m_io_res, &m_ev_irq_update,
                                &m_ev_display, &m_ev_start_stop);
    master = new fb_device_master("ma");
...
master_device *
fb_device::get_master(void)
{
    return (master_device *)master;
}

slave_device *
fb_device::get_slave(void)
{
    return (slave_device *)slave;
}

void
fb_device::start_stop_thread(void)
{
...
void 
fb_device::irq_update_thread(void)
{
    while(1){
...
fb_device::display_thread(void)
{
    ...
    while (1)
    {
        ...
        /* Pings the viewer so that it refreshes its content */
        char c = 1;
        ::write (m_pout, &c, sizeof(c));
 ...
 * fb_device_master
 */
fb_device_master::fb_device_master(const char *_name):
master_device(_name)
{
    m_crt_tid = 0;
}
...
#ifdef TRACE_EVENT_ENABLED
void 
fb_device_master::rcv_rsp(uint8_t tid, uint8_t *data, bool bErr, bool bWrite, hwe_cont* hwe_rsp)
#else
void 
fb_device_master::rcv_rsp(uint8_t tid, uint8_t *data, bool bErr, bool bWrite)
#endif
{
    if(tid != m_crt_tid)
        EPRINTF("Bad tid (%d / %d)\n", tid, m_crt_tid);
...
fb_device_slave::fb_device_slave(const char *_name, fb_io_resource_t *io_res,
                                 ...
                                 sc_event *start_stop):slave_device(_name)
{
    m_io_res        = io_res;
...
void
fb_device_slave::write(uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
...
void
fb_device_slave::read(uint32_t ofs, uint8_t be,
...
void
fb_device_slave::rcv_rqst(uint32_t ofs, uint8_t be,
