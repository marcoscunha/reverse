...
{
public:
    fb_device_master (const char *_name);
    ~fb_device_master();

public:
    /*
     *   Obtained from father
     *    void send_req(uint8_t tid, uint32_t addr, uint8_t *data, 
     *      uint8_t bytes, bool bWrite);
     */
#ifdef TRACE_EVENT_ENABLED
...
public:
    ...
    fb_device (sc_module_name _name, fb_reset_t *reset_status);
    virtual ~fb_device ();

public:
    slave_device  *get_slave(void);
    master_device *get_master(void);
    static void close_ramdacs(void);
...
private:
    ...
    static uint8_t   *shm_addr[128][2];
    static int        shmids[128][2];
    static int        nb_fb;
