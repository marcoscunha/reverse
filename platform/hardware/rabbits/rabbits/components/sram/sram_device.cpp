...
/**
* Name: sram_device
* Function: allocate memory area for the SRAM and fill it with zeros
* Input: _size: memory size
*/
...
/**
* Name: ~sram_device
* Function: delete memory area previously allocated for the SRAMs
*/
...
/**
* Name: write
* Function: write data to the SRAM memory
* Input: ofs: destination offset
*        be: byte enable
*        data: input data
*        bErr: true if error, else false
*/
void 
sram_device::write (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
    ...
    if(ofs > size || be == 0)
      ...
      /// depending on 'byte enable' (be) select part of the word which will be written
    if(!err){
        switch(be){
            ///byte access
        ...
        if (!err)
            switch (lwid)
            {
            case 1: /// write a byte to the SRAM enabled byte
                *((uint8_t *)(mem + ofs) + loffs) =
                    *((uint8_t *)data + loffs);
                break;
            case 2: /// write a word to the SRAM enable word
                *((uint16_t *) (mem + ofs) + loffs) =
                    *((uint16_t *) data + loffs);
                break;
            case 4: /// write a dword...
                *((uint32_t *) (mem + ofs) + loffs) =
                    *((uint32_t *) data + loffs);
                break;
            case 8: /// write a qword...
...
/**
* Name: read
* Function: read data from the SRAM memory (from adress: mem+ofs)
* Input: ofs: SRAM memory ofset (to be added to base address)
*        be: byte enable
*        data: destination data
*        bErr: true if error, else false
*/
void sram_device::read (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
    int loffs = 0; /// data offset
    int lwid  = 0; /// access width
    int err   = 0;
...
void
sram_device::rcv_rqst(uint32_t ofs, uint8_t be,
