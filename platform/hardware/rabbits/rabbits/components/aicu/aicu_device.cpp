...
void aicu_device::write (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
    uint32_t                value;
    ...
    if(ofs < AICU_LOCAL){
        ...
        case AICU_HANDLER30 :
        case AICU_HANDLER31 :
            if ((int) (ofs - AICU_HANDLER0) < this->L + this->G)
            {
...
void aicu_device::read (uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
...
void aicu_device::rcv_rqst (uint32_t ofs, uint8_t be,
