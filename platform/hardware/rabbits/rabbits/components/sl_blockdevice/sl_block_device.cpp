...
sl_block_device::sl_block_device(sc_module_name _name, const char *fname,
                                 uint32_t block_size):
sc_module(_name)
{
    ...
    master = new sl_block_device_master("ma");
    slave = new sl_block_device_slave("sl", m_cs_regs, &ev_op_start,
                                      &ev_irq_update);
...
void
sl_block_device::open_host_file(const char *fname)
{
    if (m_fd != -1){
        ::close (m_fd);
        m_fd = -1;
...
master_device *
sl_block_device::get_master(void)
{
    return (master_device *)master;
}

slave_device *
sl_block_device::get_slave(void)
{
    return (slave_device *)slave;
}

void
sl_block_device::control_thread(void)
{
...
void
sl_block_device::irq_update_thread(void)
{
    while(1){
 ...
 */
sl_block_device_master::sl_block_device_master(const char *_name):
master_device(_name)
{
    m_crt_tid = 0;
...
#ifdef TRACE_EVENT_ENABLED
void 
sl_block_device_master::rcv_rsp(uint8_t tid, uint8_t *data, bool bErr,
...
void
sl_block_device_slave::write(uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
...
void
sl_block_device_slave::read(uint32_t ofs, uint8_t be, uint8_t *data, bool &bErr)
{
...
void
sl_block_device_slave::rcv_rqst(uint32_t ofs, uint8_t be, uint8_t *data,
                                bool bWrite)
{
