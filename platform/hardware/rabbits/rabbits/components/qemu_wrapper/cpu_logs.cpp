...
static int      s_pid_chrono_cpu_fvs[256];
...
{
    #if defined(TIME_AT_FV_LOG_FILE) || defined (TIME_AT_FV_LOG_GRF)
    int                     i, cpu;
    #endif
    ...
    {
        double                  s1, s2, diff, max_fq;
        unsigned char           val[32];

        max_fq = (double) m_cpu_fvs[m_cpu_nb_fv_levels - 1];
        ...
        for (cpu = m_ncpu - 1; cpu >= 0; cpu--)
        {
            ...
            for (i = 0; i < m_cpu_nb_fv_levels_1; i++)
            {
                ...
                s1 += diff * (m_cpu_fvs[i] * 100 / max_fq);
                s2 += diff;
            }
