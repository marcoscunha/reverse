...
#define dnp_reg_write(idx, val) \
  cpu_write(UINT32, DNP_BASE_ADDR + idx, val)

#define dnp_reg_read(idx, ptr) \
  cpu_read(UINT32, DNP_BASE_ADDR + idx, ptr)
...
struct engine_cmd {
	...
	unsigned dest_dev  :1;
	/*This field MUST contains one of the previous constant: e.g destdev may be equal to 1, then Master
	  M1 is  selected to perform the  command*/
	unsigned dest_z  	:6;
	unsigned dest_y  	:6;
	unsigned dest_x  	:6;
	unsigned pad1     :13;
...
/************************3RD WORD**************************************************/  
	...
	unsigned cmd      :8;
        // This field MUST contains one of the previous constant: e.g cmd may be uquals to 4, then is a DNP_CMD_RDMA_GET
