...
#define  DNA_THREAD_READY      0xFACE
#define  DNA_THREAD_RUNNING    0xBEEF
#define  DNA_THREAD_SLEEPING   0xBEDD
#define  DNA_THREAD_SUSPENDED  0xD15A
#define  DNA_THREAD_WAITING    0xB10C
#define  DNA_THREAD_ENDED      0xDEAD
 ...
 */
#ifdef WRITEBACK
typedef uint32_t thread_resource_t;
#define  DNA_NO_RESOURCE         0
#define  DNA_RESOURCE_SEMAPHORE  1
#define  DNA_RESOURCE_THREAD     2
#else
typedef enum
{
  DNA_NO_RESOURCE,
...
typedef struct _thread_info
{
  ...
  int32_t cpu_id __attribute__((aligned(32)));
  int32_t group;
  int32_t affinity;
  int32_t sem_tokens;

  thread_status_t status __attribute__((aligned(32)));
  int32_t spare1[7];

  thread_resource_t resource __attribute__((aligned(32))); 
  int32_t spare2[7];
  int32_t resource_id;
