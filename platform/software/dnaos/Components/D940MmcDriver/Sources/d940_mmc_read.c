...
status_t d940_mmc_read (void * handler, void * destination,
    int64_t offset, int32_t * p_count)
{
  ...
  watch (status_t)
  {
    ...
    status = d940_mmc_driver . mmc -> execute (d940_mmc_driver . card,
        MMC_READ, destination, block_offset, block_count);
