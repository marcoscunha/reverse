...
class APECObjectFile < APEObjectFile 
  attr :object
  attr :flags
  attr :interface
  attr :version
  ...
  def APECObjectFile.envs()
    return [ 'APES_COMPILER', 'APES_CC_FLAGS', 'APES_CC_OPTIMIZATIONS' ]
  end

  def APECObjectFile.createWith(source, interface, cache, includes)
    interface_var = interface.id.name.upcase + '_CC_FLAGS'
    flags = ENV['APES_CC_FLAGS'] + ' ' + 
      ENV['APES_CC_OPTIMIZATIONS'] + ' ' +
      (ENV[interface_var] != nil ? ENV[interface_var] : "");
    return super(source, interface, cache, includes, flags, APECObjectFile)
  end
