...
void slave()
{
   ...
   LOCK(locks->idlock)
     DCACHE_INVAL(&global->id,sizeof(long));
     procid = global->id;
     global->id = global->id+1;
     DCACHE_FLUSH(&global->id,sizeof(long));
   UNLOCK(locks->idlock)
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute
   ...
   */
   ...
   DCACHE_INVAL(&im, sizeof(long));
   for (i=0;i<im;i++) {
     t1a = (double *) t2a[i];
     t1b = (double *) t2b[i];
     DCACHE_INVAL(&jm, sizeof(long));
     for (j=0;j<jm;j++) {
        t1a[j] = 0.0;
        t1b[j] = 0.0;
     }
...
/* every process gets its own copy of the timing variables to avoid
   ...
   DCACHE_INVAL(&ysca1, sizeof(double));
   ysca1 = 0.5*ysca;
   if (procid == MASTER) {
     t1a = (double *) f;
     for (iindex = 0;iindex<=jmx[numlev-1]-1;iindex++) {
       ...
       DCACHE_FLUSH(&t1a[iindex],sizeof(double));
     }
   }
   ...
   if (gp[procid].neighbors[UP] == -1) {
     ...
     DCACHE_FLUSH(&t1a[firstcol],sizeof(double)*((lastcol-firstcol)+1));
   }
   ...
   if (gp[procid].neighbors[LEFT] == -1) {
     ...
     DCACHE_FLUSH(&t2a[firstrow][0], sizeof(double)*((lastrow-firstrow)+1));
   }
   if (gp[procid].neighbors[RIGHT] == -1) {
     for(j=firstrow;j<=lastrow;j++) {
       t2a[j][jm-1] = 0.0;
     }
     DCACHE_FLUSH(&t2a[firstrow][jm-1], sizeof(double)*((lastrow-firstrow)+1));
   }
...
/* compute psib array (one-time computation) and integrate into psibi */
   ...
   if (gp[procid].neighbors[LEFT] == -1) {
     for(i=istart;i<=iend;i++) {
       t2a[i][0] = t2b[i][0];
       DCACHE_FLUSH(&t2a[i][0],sizeof(double));
     }
   }
   if (gp[procid].neighbors[RIGHT] == -1) {
     for(i=istart;i<=iend;i++) {
       t2a[i][jm-1] = t2b[i][jm-1];
       DCACHE_FLUSH(&t2a[i][jm-1],sizeof(double));
     }
   }
#if defined(MULTIPLE_BARRIERS)
   ...
   for(i=ist;i<=ien;i++) {
     ...
     DCACHE_FLUSH(&t1a[jst], sizeof(double)*((jen-jst)+1));
   }
   ...
   for(i=istart;i<=iend;i++) {
     ...
     DCACHE_FLUSH(&t1b[jstart], sizeof(double)*((jend-jstart)+1));
   }
#if defined(MULTIPLE_BARRIERS)
   BARRIER(bars->sl_prini,nprocs)
#else
...
/* update the shared variable psibi by summing all the psibiprivs
   ...
   LOCK(locks->psibilock)
     ...
     DCACHE_FLUSH(&global->psibi, sizeof(double));
...
/* initialize psi matrices the same way  */
   ...
   for(psiindex=0;psiindex<=1;psiindex++) {
     ...
     if ((gp[procid].neighbors[UP] == -1) && (gp[procid].neighbors[LEFT] == -1)) {
       DCACHE_INVAL(&t2a[0],sizeof(double)); 
       t2a[0][0] = 0.0;
     }
     if ((gp[procid].neighbors[UP] == -1) && (gp[procid].neighbors[RIGHT] == -1)) {
       t2a[0][jm-1] = 0.0;
     ...
     for(i=firstrow;i<=lastrow;i++) {
       DCACHE_INVAL(&t2a[i],sizeof(double));
       t1a = (double *) t2a[i];
       for(iindex=firstcol;iindex<=lastcol;iindex++) {
         t1a[iindex] = 0.0;
       }
       DCACHE_FLUSH(t1a, sizeof(double)*((lastcol-firstcol)+1));
     }
   }
...
/* update values of psilm array to psilm + psim[2]  */
  ...
  if ((procid == MASTER) || (do_stats)) {
    ...
    DCACHE_FLUSH(&gp[procid].total_time,sizeof(double)); 
  }
}
