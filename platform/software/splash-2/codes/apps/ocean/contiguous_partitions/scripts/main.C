...
int main(int argc, char *argv[])
{
   ...
   DCACHE_FLUSH(&numlev, sizeof(long));
   ...
   if (imx == NULL) printf("Error G_MALLOC\n",tolerance);

   jmx = (long *) G_MALLOC(numlev*sizeof(long));
   if (jmx == NULL) printf("Error G_MALLOC\n",tolerance);

   lev_res = (double *) G_MALLOC(numlev*sizeof(double));
   DCACHE_FLUSH(&lev_res,sizeof(double*));
   lev_tol = (double *) G_MALLOC(numlev*sizeof(double));
   DCACHE_FLUSH(&lev_tol,sizeof(double*));
   ...
   imx[numlev-1] = im;
   DCACHE_FLUSH(&imx[numlev-1],sizeof(long));
   jmx[numlev-1] = jm;
   DCACHE_FLUSH(&jmx[numlev-1],sizeof(long));
   lev_res[numlev-1] = res;
   DCACHE_FLUSH(&lev_res[numlev-1],sizeof(double));
   lev_tol[numlev-1] = tolerance;
   DCACHE_FLUSH(&lev_tol[numlev-1],sizeof(double));

   for (i=numlev-2;i>=0;i--) {
       imx[i] = ((imx[i+1] - 2) / 2) + 2;
       DCACHE_FLUSH(&imx[i],sizeof(long));
       jmx[i] = ((jmx[i+1] - 2) / 2) + 2;
       DCACHE_FLUSH(&jmx[i],sizeof(long));
       lev_res[i] = lev_res[i+1] * 2;
       DCACHE_FLUSH(&lev_res[i],sizeof(double));
   }
   ...
   DCACHE_FLUSH(xpts_per_proc,sizeof(long)*numlev);
   DCACHE_FLUSH(ypts_per_proc,sizeof(long)*numlev);
   ...
   DCACHE_FLUSH(&psi,       sizeof(double****));

   psim = (double ****) G_MALLOC(d_size);
   DCACHE_FLUSH(&psim,       sizeof(double****));

   work1 = (double ****) G_MALLOC(d_size);
   DCACHE_FLUSH(&work1,       sizeof(double****));

   work4 = (double ****) G_MALLOC(d_size);
   DCACHE_FLUSH(&work4,       sizeof(double****));

   work5 = (double ****) G_MALLOC(d_size);
   DCACHE_FLUSH(&work5,       sizeof(double****));

   work7 = (double ****) G_MALLOC(d_size);
   DCACHE_FLUSH(&work7,       sizeof(double****));

   temparray = (double ****) G_MALLOC(d_size);
   DCACHE_FLUSH(&temparray,       sizeof(double****));


   d_size = 2*sizeof(double **);
   for (i=0;i<nprocs;i++) {
     psi[i] = (double ***) G_MALLOC(d_size);
     DCACHE_FLUSH(&psi[i],       sizeof(double***));

     psim[i] = (double ***) G_MALLOC(d_size);
     DCACHE_FLUSH(&psim[i],      sizeof(double***));

     work1[i] = (double ***) G_MALLOC(d_size);
     DCACHE_FLUSH(&work1[i],     sizeof(double***));

     work4[i] = (double ***) G_MALLOC(d_size);
     DCACHE_FLUSH(&work4[i],     sizeof(double***));

     work5[i] = (double ***) G_MALLOC(d_size);
     DCACHE_FLUSH(&work5[i],     sizeof(double***));

     work7[i] = (double ***) G_MALLOC(d_size);
     DCACHE_FLUSH(&work7[i],     sizeof(double***));

     temparray[i] = (double ***) G_MALLOC(d_size);
     DCACHE_FLUSH(&temparray[i], sizeof(double***));
   }


   d_size = nprocs*sizeof(double **);
   psium = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&psium, sizeof(double***));

   psilm = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&psilm, sizeof(double***));

   psib = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&psib, sizeof(double***));

   ga = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&ga, sizeof(double***));

   gb = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&gb, sizeof(double***));

   work2 = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&work2, sizeof(double***));

   work3 = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&work3, sizeof(double***));

   work6 = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&work6, sizeof(double***));

   tauz = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&tauz, sizeof(double***));

   oldga = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&oldga, sizeof(double***));

   oldgb = (double ***) G_MALLOC(d_size);
   DCACHE_FLUSH(&oldgb, sizeof(double***));


   gp = (struct Global_Private *) G_MALLOC((nprocs+1)*sizeof(struct Global_Private));
   DCACHE_FLUSH(&gp, sizeof(struct Global_Private*));

   for (i=0;i<nprocs;i++) {
     gp[i].rel_num_x = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].rel_num_x, sizeof(long*));

     gp[i].rel_num_y = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].rel_num_y, sizeof(long*));

     gp[i].eist = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].eist, sizeof(long*));

     gp[i].ejst = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].ejst, sizeof(long*));

     gp[i].oist = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].oist, sizeof(long*));

     gp[i].ojst = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].ojst, sizeof(long*));

     gp[i].rlist = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].rlist, sizeof(long*));

     gp[i].rljst = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].rljst, sizeof(long*));

     gp[i].rlien = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].rlien, sizeof(long*));

     gp[i].rljen = (long *) G_MALLOC(numlev*sizeof(long));
     DCACHE_FLUSH(&gp[i].rljen, sizeof(long*));

     gp[i].multi_time = 0;
     DCACHE_FLUSH(&gp[i].multi_time, sizeof(double));

     gp[i].total_time = 0;
     DCACHE_FLUSH(&gp[i].total_time, sizeof(double));
   }
   ...
   DCACHE_FLUSH(&global, sizeof(struct global_struct*));
   for (i=0;i<nprocs;i++) {
     psi[i][0] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&psi[i][0], sizeof(double**)); 

     psi[i][1] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&psi[i][1], sizeof(double**)); 

     psim[i][0] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&psim[i][0], sizeof(double**)); 

     psim[i][1] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&psim[i][1], sizeof(double**)); 

     psium[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&psium[i], sizeof(double**)); 

     psilm[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&psilm[i], sizeof(double**)); 

     psib[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&psib[i], sizeof(double**)); 

     ga[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&ga[i], sizeof(double**)); 

     gb[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&gb[i], sizeof(double**)); 
     
     work1[i][0] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work1[i][0], sizeof(double**)); 

     work1[i][1] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work1[i][1], sizeof(double**)); 

     work2[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work2[i], sizeof(double**)); 

     work3[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work3[i], sizeof(double**)); 

     work4[i][0] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work4[i][0], sizeof(double**)); 

     work4[i][1] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work4[i][1], sizeof(double**)); 

     work5[i][0] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work5[i][0], sizeof(double**)); 

     work5[i][1] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work5[i][1], sizeof(double**)); 

     work6[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work6[i], sizeof(double**)); 

     work7[i][0] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work7[i][0], sizeof(double**)); 

     work7[i][1] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&work7[i][1], sizeof(double**)); 

     temparray[i][0] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&temparray[i][0], sizeof(double**)); 

     temparray[i][1] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&temparray[i][1], sizeof(double**)); 

     tauz[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&tauz[i], sizeof(double**)); 

     oldga[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&oldga[i], sizeof(double**)); 

     oldgb[i] = (double **) G_MALLOC(d_size);
     DCACHE_FLUSH(&oldgb[i], sizeof(double**)); 
...
//   locks = (struct locks_struct *) G_MALLOC(sizeof(struct locks_struct));
   locks = (struct locks_struct *) G_MALLOC(sizeof(struct locks_struct)+0x1F);
   locks = (((uintptr_t)locks + 0x1F) & ~(uintptr_t)0x1F); // 32 bytes aligned 
   DCACHE_FLUSH(&locks, sizeof(struct locks_struct *));

// bars = (struct bars_struct *) G_MALLOC(sizeof(struct bars_struct));
   bars = (struct bars_struct *) G_MALLOC(sizeof(struct bars_struct)+0x1F);
   bars = (((uintptr_t)bars + 0x1F) & ~(uintptr_t)0x1F); // 32 bytes aligned 

   DCACHE_FLUSH(&bars, sizeof(struct bars_struct *));
...
/*   printf("&locks             %p\n", &locks);                
   printf("locks              %p\n", locks);
   printf("&locks->idlock     %p %d\n", &locks->idlock,     locks->idlock    );   
   printf("&locks->psiailock  %p %d\n", &locks->psiailock,  locks->psiailock );
   printf("&locks->psibilock  %p %d\n", &locks->psibilock,  locks->psibilock );
   printf("&locks->donelock   %p %d\n", &locks->donelock,   locks->donelock  );
   printf("&locks->error_lock %p %d\n", &locks->error_lock, locks->error_lock);
   printf("&locks->bar_lock   %p %d\n", &locks->bar_lock,   locks->bar_lock  );
*/
...
*/
   ...
   for (i=0;i<numlev;i++) {
     ...
     DCACHE_FLUSH(&i_int_coeff[i], sizeof(double));
     j_int_coeff[i] = 1.0/(jmx[i]-1);
     DCACHE_FLUSH(&j_int_coeff[i], sizeof(double));
   }
...
/* initialize constants and variables
   ...
   DCACHE_FLUSH(&eig2,sizeof(double));

   jmm1 = jm-1 ;
   DCACHE_FLUSH(&jmm1, sizeof(double));
   ysca = ((double) jmm1)*res ;
   DCACHE_FLUSH(&ysca, sizeof(double));
  
   im = (imx[numlev-1]-2)/yprocs + 2;
   DCACHE_FLUSH(&im, sizeof(long));
   jm = (jmx[numlev-1]-2)/xprocs + 2;
   DCACHE_FLUSH(&jm, sizeof(long));
