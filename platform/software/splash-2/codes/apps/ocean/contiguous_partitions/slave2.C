...
/* set values of work2 array to psi1 - psi3   */
   ...
   if (gp[procid].neighbors[LEFT] == -1) {
     for(j=firstrow;j<=lastrow;j++) {
       ...
       DCACHE_FLUSH(&t2a[j][0],sizeof(double));
     }
   }
   if (gp[procid].neighbors[RIGHT] == -1) {
     for(j=firstrow;j<=lastrow;j++) {
       t2a[j][jm-1] = t2b[j][jm-1]-t2c[j][jm-1];
       DCACHE_FLUSH(&t2a[j][jm-1],sizeof(double));
     }
   }
   for(i=firstrow;i<=lastrow;i++) {
     t1a = (double *) t2a[i];
...
/* set values of work3 array to h3/h * psi1 + h1/h * psi3  */
   ...
   if (gp[procid].neighbors[UP] == -1) {
     ...
     DCACHE_FLUSH(&t2a[0][firstcol],sizeof(double)*((lastcol-firstcol)+1));
   }
   if (gp[procid].neighbors[DOWN] == -1) {
     for(j=firstcol;j<=lastcol;j++) {
     ...
     DCACHE_FLUSH(&t2a[im-1][firstcol],sizeof(double)*((lastcol-firstcol)+1));
   }
   if (gp[procid].neighbors[LEFT] == -1) {
     for(j=firstrow;j<=lastrow;j++) {
...
/*     *******************************************************
   ...
   if (gp[procid].neighbors[LEFT] == -1) {
     for(i=istart;i<=iend;i++) {
       t2d[i][0] = t2b[i][0];
       DCACHE_FLUSH(&t2d[i][0], sizeof(double));
     }
   }
   if (gp[procid].neighbors[RIGHT] == -1) {
     for(i=istart;i<=iend;i++) {
       t2d[i][jm-1] = t2b[i][jm-1];
       DCACHE_FLUSH(&t2d[i][jm-1], sizeof(double));
     }
   }
   ...
   for(i=ist;i<=ien;i++) {
     ...
     DCACHE_FLUSH(&t1d[jst],sizeof(double)*((jen-jst)+1));
   }
...
/* after computing its private sum, every process adds that to the
   ...
   DCACHE_FLUSH(&global->psiai, sizeof(double));
...
/*      *******************************************************
...
#ifdef DEBUG_PHASES
   printf("Slave 2 - 8th Phase\n");
#endif

   DCACHE_INVAL(&global->psiai,sizeof(double)); 
   DCACHE_INVAL(&global->psibi,sizeof(double)); 
   ...
   if (gp[procid].neighbors[UP] == -1) {
     ...
     for(j=jstart;j<=jend;j++) {
       t1d[j] = t1b[j];
       DCACHE_FLUSH(&t1d[j], sizeof(double));
     }
   }
   if (gp[procid].neighbors[DOWN] == -1) {
     t1d = (double *) t2d[im-1];
   ...
   if ((procid == MASTER) || (do_stats)) {
     CLOCK(multi_end);
     gp[procid].multi_time += (multi_end - multi_start);
     DCACHE_FLUSH(&gp[procid].multi_time, sizeof(double));
   }
