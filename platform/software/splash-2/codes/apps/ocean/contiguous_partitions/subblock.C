...
/* each direction                                                  */
   ...
   for (i=0;i<numlev;i++) {
     ...
     for (j=0;j<xprocs;j++) {
       for (k=0;k<yprocs;k++) {
         ...
         DCACHE_FLUSH(&gp[k*xprocs+j].rel_num_x[i],sizeof(long));
       }
     }
     ...
     for (j=0;j<yprocs;j++) {
       for (k=0;k<xprocs;k++) {
         ...
         DCACHE_FLUSH(&gp[j*xprocs+k].rel_num_y[i], sizeof(long));
       }
     }
   }
   ...
   for (my_num=0;my_num<nprocs;my_num++) {
     for (i=0;i<numlev;i++) {
         gp[my_num].rlist[i] = 1;
         DCACHE_FLUSH(&gp[my_num].rlist[i], sizeof(long));
         gp[my_num].rljst[i] = 1;
         DCACHE_FLUSH(&gp[my_num].rljst[i], sizeof(long));
         gp[my_num].rlien[i] = gp[my_num].rlist[i] + gp[my_num].rel_num_y[i];
         DCACHE_FLUSH(&gp[my_num].rlien[i], sizeof(long));
         gp[my_num].rljen[i] = gp[my_num].rljst[i] + gp[my_num].rel_num_x[i];
         DCACHE_FLUSH(&gp[my_num].rljen[i], sizeof(long));
         gp[my_num].eist[i] = gp[my_num].rlist[i] + 1;
         DCACHE_FLUSH(&gp[my_num].eist[i], sizeof(long));
         gp[my_num].oist[i] = gp[my_num].rlist[i];
         DCACHE_FLUSH(&gp[my_num].oist[i], sizeof(long));
         gp[my_num].ejst[i] = gp[my_num].rljst[i] + 1;
         DCACHE_FLUSH(&gp[my_num].ejst[i], sizeof(long));
         gp[my_num].ojst[i] = gp[my_num].rljst[i];
         DCACHE_FLUSH(&gp[my_num].ojst[i], sizeof(long));
     }
   }
  for (i=0;i<nprocs;i++) {
      gp[i].neighbors[LEFT] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[LEFT]     , sizeof(long)); 
      gp[i].neighbors[RIGHT] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[RIGHT]    , sizeof(long));
      gp[i].neighbors[UP] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[UP]       , sizeof(long)); 
      gp[i].neighbors[DOWN] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[DOWN]     , sizeof(long));
      gp[i].neighbors[UPLEFT] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[UPLEFT]   , sizeof(long));     
      gp[i].neighbors[UPRIGHT] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[UPRIGHT]  , sizeof(long)); 
      gp[i].neighbors[DOWNLEFT] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[DOWNLEFT] , sizeof(long));
      gp[i].neighbors[DOWNRIGHT] = -1;
      DCACHE_FLUSH(&gp[i].neighbors[DOWNRIGHT], sizeof(long));
  ...
  for (i=0;i<nprocs;i++) {
    ...
    DCACHE_FLUSH(&gp[i].rownum,sizeof(long));
    gp[i].colnum = i%xprocs;
    DCACHE_FLUSH(&gp[i].colnum,sizeof(long));
  }
}
