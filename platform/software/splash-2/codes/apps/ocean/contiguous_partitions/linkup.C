...
void linkup(double **row_ptr)
{
  ...
  DCACHE_FLUSH(row_ptr, sizeof(double*)*y_part);
}
...
void link_multi()
{
  ...
  for (j=0;j<nprocs;j++) {
    ...
    for (i=0;i<numlev;i++) {
      ...
      DCACHE_FLUSH(&q_multi[j][i],sizeof(double **));
      zz += d_size;
    }
  }
  ...
  for (l=0;l<numlev;l++) {
    ...
    for (j=0;j<nprocs;j++) {
      ...
      DCACHE_FLUSH(q_multi[j][l],sizeof(double*)*y_part);
    }
  }
  ...
  for (i=0;i<nprocs;i++) {
    ...
    DCACHE_FLUSH(&rhs_multi[i],sizeof(double ***));
    z += d_size;
  }
  for (j=0;j<nprocs;j++) {
    ...
    for (i=0;i<numlev;i++) {
      ...
      DCACHE_FLUSH(&rhs_multi[j][i], sizeof(double**)); 
      zz += d_size;
    }
  }
  ...
  for (l=0;l<numlev;l++) {
    ...
    for (j=0;j<nprocs;j++) {
      ...
      DCACHE_FLUSH(rhs_multi[j][l], sizeof(double*)*y_part);
    }
  }
