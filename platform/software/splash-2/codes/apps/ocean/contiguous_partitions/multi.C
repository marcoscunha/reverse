...
void multig(long my_id)
{
   ...
   flag2 = 0;
   iter = 0;
   DCACHE_INVAL(numlev,sizeof(long));
   m = numlev-1;
   wmax = maxwork;
   my_num = my_id;
   wu = 0.0;
   ...
   while ((!flag1) && (!flag2)) {
     ...
     if (my_num == MASTER) {
       multi->err_multi = 0.0;
       DCACHE_FLUSH(&multi->err_multi,sizeof(double));
     }

   /* dna_printf(" MULTI FIRST BARRIER - START\n ");*/
...
/* update the global error if necessary                         */
     ...
     DCACHE_INVAL(&local_err, sizeof(double));
     DCACHE_INVAL(&multi->err_multi, sizeof(double));
...
/* and reset global->err before we read it and decide what to do   */
     ...
     BARRIER(bars->barrier,nprocs)
#endif
     DCACHE_INVAL(&lev_tol[k], sizeof(double));
...
/* transfer residual to rhs of coarser grid                        */
           ...
           DCACHE_FLUSH(&lev_tol[k-1],sizeof(double));
           k = k-1;
           putz(k,my_num);
...
/* others do a black				                     */
   ...
   DCACHE_INVAL(&rhs_multi[my_num], sizeof(double)*k);
   ...
   if (color == RED_ITER) {
     for (i=evenistart;i<iend;i+=2) {
       ...
       for (j=evenjstart;j<jend;j+=2) {
         ...
         t1a[j] = newval;
         DCACHE_FLUSH(&t1a[j], sizeof(double));
     ...
     for (i=evenistart;i<iend;i+=2) {
       ...
       for (j=oddjstart;j<jend;j+=2) {
	     ...
	     t1b[j] ;
         oldval = t1a[j];
         DCACHE_FLUSH(&oldval, sizeof(double));
         newval = a / factor;
         DCACHE_FLUSH(&newval, sizeof(double));
         newerr = oldval - newval;
         DCACHE_FLUSH(&newerr, sizeof(double));
         t1a[j] = newval;
         DCACHE_FLUSH(&t1a[j], sizeof(double));
         if (fabs(newerr) > maxerr) {
           maxerr = fabs(newerr);
...
void rescal(long kf, long my_num)
{
   ...
   DCACHE_INVAL(&q_multi[my_num][kf],sizeof(double));
...
/* do bilinear interpolation */
	  ...
	  if (if17 == 2) {
        DCACHE_INVAL(&q_multi[up_proc][kf][im-1][jf],sizeof(double));
	  ...
	  if (if17 == 2) {
          DCACHE_INVAL(&q_multi[up_proc][kf][im-1][jf-2], sizeof(double));
       ...
       DCACHE_FLUSH(&t1c[jc], sizeof(double));
     }
   }
}
...
void intadd(long kc, long my_num)
{
   ...
   for(ic=istart;ic<=iend;ic++) {
     ...
     DCACHE_FLUSH(&t1d[(2*jstart)-1],sizeof(double)*((jend-jstart)+1)*2);
     DCACHE_FLUSH(&t1e[(2*jstart)-1],sizeof(double)*((jend-jstart)+1)*2);
   }
}
...
void putz(long k, long my_num)
{
   ...
   DCACHE_INVAL(&q_multi[my_num], sizeof(double**)*k);
   DCACHE_INVAL(&q_multi[my_num][k], sizeof(double*));
   t2a = (double **) q_multi[my_num][k];
   for (i=istart;i<=iend;i++) {
     ...
     DCACHE_FLUSH(t1a, sizeof(double)*((jend-jstart)+1));
   }
}
  ...
  double *t1b;
   ...
   if (gp[pid].neighbors[UP] == -1) {
     ...
     if (jj != -1) {
       ...
       jj = gp[pid].neighbors[DOWN];
       if (jj != -1) {
         DCACHE_INVAL(&q_multi[jj][k][1][jm-1], sizeof(double));
     ...
     if (jj != -1) {
       ...
       jj = gp[pid].neighbors[UP];
       if (jj != -1) {
         DCACHE_INVAL(&q_multi[jj][k][im-2][jm-1], sizeof(double));
   ...
   if (j != -1) {
     ...
     for (i=1;i<=lastcol;i++) {
       DCACHE_INVAL(&t1b[i], sizeof(double));
       t1a[i] = t1b[i];
     }
   }
...
void copy_red(long k, long procid)
{
   ...
   DCACHE_INVAL(&q_multi[procid],sizeof(double *)*k);
   t2a = (double **) q_multi[procid][k];
   j = gp[procid].neighbors[UP];
   if (j != -1) {
     t1a = (double *) t2a[0];
     DCACHE_INVAL(&q_multi[j][k][im-2],sizeof(double));
     t1b = (double *) q_multi[j][k][im-2];
    DCACHE_INVAL(t1b,sizeof(double)*lastcol);
    for (i=2;i<=lastcol;i+=2) {
       DCACHE_INVAL(&t1b[i],sizeof(double));
       t1a[i] = t1b[i];
     }
   ...
   if (j != -1) {
     t1a = (double *) t2a[im-1];
     DCACHE_INVAL(&q_multi[j][k][1],sizeof(double*));
     t1b = (double *) q_multi[j][k][1];
     DCACHE_INVAL(t1b,sizeof(double)*lastcol);
...
void copy_black(long k, long procid)
{
   ...
   if (j != -1) {
     ...
     for (i=2;i<=lastcol;i+=2) {
       DCACHE_INVAL(&t1b[i],sizeof(double));
       t1a[i] = t1b[i];
       DCACHE_FLUSH(&t1a[i],sizeof(double));
     }
   }
   j = gp[procid].neighbors[LEFT];
   if (j != -1) {
