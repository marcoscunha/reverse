...
void INTRAF(double *VIR, long ProcID)
{
    ...
    for (mol = StartMol[ProcID]; mol < StartMol[ProcID+1];  mol++) {
        ...
        for (dir = XDIR; dir <= ZDIR; dir++) {
            ...
            DCACHE_INVAL(&C2, sizeof(double));
            VAR[mol].VM[dir] = C1 * temp_p[O]
                + C2 * (temp_p[H1] +
                        temp_p[H2] );
            DCACHE_FLUSH(&VAR[mol].VM[dir], sizeof(double));
        ...
        for (dir = XDIR; dir <= ZDIR; dir++) {
            ...
            DCACHE_INVAL(&T1, sizeof(double));
            DCACHE_INVAL(&T2, sizeof(double));
    ...
    for (mol = StartMol[ProcID]; mol < StartMol[ProcID+1];  mol++)
        for ( dir = XDIR; dir <= ZDIR; dir++)
            for (atom = 0; atom < NATOM; atom++){
                DCACHE_INVAL(&VAR[mol].F[DISP][dir][atom], sizeof(double));
                DCACHE_INVAL(&VAR[mol].F[FORCES][dir][atom] ,sizeof(double));
               LVIR += VAR[mol].F[DISP][dir][atom] *
                    VAR[mol].F[FORCES][dir][atom];
                DCACHE_FLUSH(&LVIR, sizeof(double));
            }
