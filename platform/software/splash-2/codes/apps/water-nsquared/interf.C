...
void INTERF(long DEST, double *VIR, long ProcID)
{
        ...
        for (ct1 = 0; ct1<NMOL; ct1++)
            for (ct2 = 0; ct2<NDIR; ct2++)
                for (ct3 = 0; ct3<NATOM; ct3++){
                    ...
                    DCACHE_FLUSH(&PFORCES[ProcID][ct1][ct2][ct3], sizeof(double));
                }
    ...
    DCACHE_INVAL(VIR, sizeof(double)); 
    *VIR = *VIR + LVIR;
    DCACHE_FLUSH(VIR, sizeof(double)); 
    ...
    if (comp_last > NMOL1) {
        for (mol = StartMol[ProcID]; mol < NMOL; mol++) {
            ...
            for ( dir = XDIR; dir  <= ZDIR; dir++) {
                ...
                DCACHE_INVAL(&temp_p[H1], sizeof(double)); 
                temp_p[H1] += PFORCES[ProcID][mol][dir][H1];
                DCACHE_FLUSH(&temp_p[H1], sizeof(double));

                DCACHE_INVAL(&temp_p[O], sizeof(double)); 
                temp_p[O]  += PFORCES[ProcID][mol][dir][O];
                DCACHE_FLUSH(&temp_p[O], sizeof(double)); 

                DCACHE_INVAL(&temp_p[H2], sizeof(double)); 
                temp_p[H2] += PFORCES[ProcID][mol][dir][H2];
                DCACHE_FLUSH(&temp_p[H2], sizeof(double)); 
            }
    ...
    DCACHE_INVAL(&gl->InterfBar,sizeof(((struct GlobalMemory *)0)->InterfBar));
    ...
    for (mol = StartMol[ProcID]; mol < StartMol[ProcID+1]; mol++) {
        for ( dir = XDIR; dir  <= ZDIR; dir++) {
            DCACHE_INVAL(&VAR[mol].F[DEST][dir] , sizeof(double));
            temp_p = VAR[mol].F[DEST][dir];
            DCACHE_FLUSH(&temp_p    , sizeof(double)); 
...
void UPDATE_FORCES(long mol, long comp, double *XL, double *YL, double *ZL, double *FF, long ProcID)
{
    ...
    DCACHE_FLUSH(&tx_p[H1], sizeof(double));

    DCACHE_INVAL(&tx_p[O],sizeof(double));
    DCACHE_INVAL(&G110[XDIR], sizeof(double));
    DCACHE_INVAL(&GG[11][XDIR], sizeof(double));
    DCACHE_INVAL(&C1          , sizeof(double));
    DCACHE_INVAL(&G23[XDIR], sizeof(double));
    tx_p[O] +=
        G110[XDIR] + GG[11][XDIR] +GG[12][XDIR]+C1*G23[XDIR];
    DCACHE_FLUSH(&tx_p[0], sizeof(double));

    tx_p[H2] +=
        GG[8][XDIR]+GG[9][XDIR]+GG[14][XDIR]+TT[XDIR]+GG[5][XDIR];
    DCACHE_FLUSH(&tx_p[H2], sizeof(double));

    ty_p[H1] +=
        GG[6][YDIR]+GG[7][YDIR]+GG[13][YDIR]+TT[YDIR]+GG[4][YDIR];
    DCACHE_FLUSH(&ty_p[H1], sizeof(double));

    DCACHE_INVAL(&ty_p[O]     , sizeof(double));
    DCACHE_INVAL(&G110[YDIR]  , sizeof(double));
    DCACHE_INVAL(&GG[12][YDIR], sizeof(double));
    DCACHE_INVAL(&G23[YDIR]   , sizeof(double));
    ty_p[O]  +=
        G110[YDIR]+GG[11][YDIR]+GG[12][YDIR]+C1*G23[YDIR];
    DCACHE_FLUSH(&ty_p[0], sizeof(double));
 
    ty_p[H2] +=
        GG[8][YDIR]+GG[9][YDIR]+GG[14][YDIR]+TT[YDIR]+GG[5][YDIR];
    DCACHE_FLUSH(&ty_p[H2], sizeof(double));

    tz_p[H1] +=
        GG[6][ZDIR]+GG[7][ZDIR]+GG[13][ZDIR]+TT[ZDIR]+GG[4][ZDIR];
     DCACHE_FLUSH(&tz_p[H1], sizeof(double));
  
    DCACHE_INVAL(&tz_p[O]     , sizeof(double));
    DCACHE_INVAL(&G110[ZDIR]  , sizeof(double));
    DCACHE_INVAL(&GG[12][ZDIR], sizeof(double));
    DCACHE_INVAL(&G23[ZDIR]   , sizeof(double));
    tz_p[O]  +=
        G110[ZDIR]+GG[11][ZDIR]+GG[12][ZDIR]+C1*G23[ZDIR];
    DCACHE_FLUSH(&tz_p[0], sizeof(double));

    tz_p[H2] +=
        GG[8][ZDIR]+GG[9][ZDIR]+GG[14][ZDIR]+TT[ZDIR]+GG[5][ZDIR];
    DCACHE_FLUSH(&tz_p[H2], sizeof(double));
