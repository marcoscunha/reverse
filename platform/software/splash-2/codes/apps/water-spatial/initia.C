...
//  static double * static_var = (double *) SOCLIB_STATIC_MEM ;
//	double * static_var = malloc(sizeof(double)); 	
//    int32_t retval = 0; 
...
//    int16_t fd;
//    if( vfs_open("/work/cunha/devel/Rabbits/platforms/bunny/random.in", 0, 777, &fd) != DNA_OK){
//        fprintf(stderr,"Error in opening file random.in\n");
//        fflush(stderr);
//        exit(-1);
//    }
//   random_numbers = fdopen(fd, "r+");
//	printf("fd = %d\n",fd);
//	printf("random_numbers = %p\n",random_numbers);
...
//			link_ptr->mol.f[vel][zdir][atom] = *static_var;
                			...
                			link_ptr->mol.f[vel][xdir][atom] = g_random[atom][0];
                			link_ptr->mol.f[vel][ydir][atom] = g_random[atom][1];
                			link_ptr->mol.f[vel][zdir][atom] = g_random[atom][2];
#endif
...
//
            ...
            printf("%d %lf %lf %lf\n", line, random[line][0],g_random[line][1],g_random[line][2]);
            line++;
            ...
            for (j=0; j < NS; j+=1) {
                ...
                for (k = 0; k < NS; k++) {
                    ...
                    for (atom = 0; atom < NATOMS; atom++) {
                        ...
                        DCACHE_FLUSH(&link_ptr->mol.F[DISP][XDIR][atom], sizeof(double));
                        link_ptr->mol.F[DISP][YDIR][atom] = YT[atom+1];
                        DCACHE_FLUSH(&link_ptr->mol.F[DISP][YDIR][atom], sizeof(double));
                        link_ptr->mol.F[DISP][ZDIR][atom] = Z;
                        DCACHE_FLUSH(&link_ptr->mol.F[DISP][ZDIR][atom], sizeof(double));
...
//            printf("%d %lf %lf %lf\n", line, g_random[line][0],g_random[line][1],g_random[line][2]);
            line++;
                        ...
                        for (deriv = ACC; deriv < MAXODR; deriv++) {
                            link_ptr->mol.F[deriv][XDIR][atom] = 0.0;
                            DCACHE_FLUSH(&link_ptr->mol.F[deriv][XDIR][atom], sizeof(double));
                            link_ptr->mol.F[deriv][YDIR][atom] = 0.0;
                            DCACHE_FLUSH(&link_ptr->mol.F[deriv][YDIR][atom], sizeof(double));
                            link_ptr->mol.F[deriv][ZDIR][atom] = 0.0;
                            DCACHE_FLUSH(&link_ptr->mol.F[deriv][ZDIR][atom], sizeof(double));
                        }
                    }

                    link_ptr->next_mol = NULL;        /* Terminating link */
                    DCACHE_FLUSH(&link_ptr->next_mol, sizeof(struct link *));
                    ...
                    if (curr_ptr == NULL) {             /* No links in box yet */
                        BOX[X_INDEX][Y_INDEX][Z_INDEX].list = link_ptr;
                        DCACHE_FLUSH(&BOX[X_INDEX][Y_INDEX][Z_INDEX].list, sizeof(struct link *));
                    } else {
                        while (curr_ptr) {               /* Scan to end of list */
                            last_ptr = curr_ptr;
                            curr_ptr = curr_ptr->next_mol;
                        ...
                        last_ptr->next_mol = link_ptr;    /* Add to end of list */
                        DCACHE_FLUSH(&last_ptr->next_mol, sizeof(struct link *));
                    } /* if curr_ptr */
                ...
                while (curr_ptr) {
                    for (atom = 0; atom < NATOMS; atom++) {
                        curr_ptr->mol.F[VEL][XDIR][atom] =
                            ( curr_ptr->mol.F[VEL][XDIR][atom] -
                             SUMX) * SUX/XMAS[atom];
                        DCACHE_FLUSH(&curr_ptr->mol.F[VEL][XDIR][atom], sizeof(double));
                        curr_ptr->mol.F[VEL][YDIR][atom] =
                            ( curr_ptr->mol.F[VEL][YDIR][atom] -
                             SUMY) * SUY/XMAS[atom];
                        DCACHE_FLUSH(&curr_ptr->mol.F[VEL][YDIR][atom], sizeof(double));
                        curr_ptr->mol.F[VEL][ZDIR][atom] =
                            ( curr_ptr->mol.F[VEL][ZDIR][atom] -
                             SUMZ) * SUZ/XMAS[atom];
                        DCACHE_FLUSH(&curr_ptr->mol.F[VEL][ZDIR][atom], sizeof(double)); 
                    } /* for atom */
                    curr_ptr = curr_ptr->next_mol;
                    DCACHE_FLUSH(&curr_ptr, sizeof(double*)); 
                } /* while curr_ptr */
            }
        }
    } /* for box */
...
//    vfs_close((int16_t)fileno(random_numbers));
