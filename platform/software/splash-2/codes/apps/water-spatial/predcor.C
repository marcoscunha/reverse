...
void PREDIC(double *C, long NOR1, long ProcID)
{
    ...
    while (curr_box) {
        ...
        while (curr_ptr) {
            ...
            DCACHE_INVAL(&curr_ptr->mol, sizeof(double*));
                ...
                for ( dir = 0; dir < NDIR; dir++)
                    for ( atom = 0; atom < NATOM; atom++ ) {
                       ...
                       for ( L = func; L < NORDER; L++) {
                            DCACHE_INVAL(&curr_ptr->mol.F[L+1][dir][atom], sizeof(double));
                            DCACHE_INVAL(&C[JI], sizeof(double));
                            S += C[JI] * curr_ptr->mol.F[L+1][dir][atom]; 
                            JI++;
                        } /* for L */
                        curr_ptr->mol.F[func][dir][atom] += S;
                        DCACHE_FLUSH(&curr_ptr->mol.F[func][dir][atom], sizeof(double));
                    } /* for atom */
                JIZ += NOR1;
            } /* for func */
