...
void POTENG(double *POTA, double *POTR, double *PTRF, long ProcID)
{
    ...
    while (curr_box) {
        ...
        while (curr_ptr) {
            ...
            curr_ptr->mol.VM[XDIR] = C1 * tx_p[O] +
                C2 * (tx_p[H1] +
                      tx_p[H2]);
            DCACHE_FLUSH(&curr_ptr->mol.VM[XDIR], sizeof(double)); 

            curr_ptr->mol.VM[YDIR] = C1*ty_p[O] +
                C2*(ty_p[H1] +
                    ty_p[H2]);
            DCACHE_FLUSH(&curr_ptr->mol.VM[YDIR],sizeof(double));
            curr_ptr->mol.VM[ZDIR] = C1*tz_p[O] +
                C2*(tz_p[H1] +
                    tz_p[H2]);
            DCACHE_FLUSH(&curr_ptr->mol.VM[ZDIR], sizeof(double)); 
    ...
    DCACHE_INVAL(&gl->PotengBar,sizeof(((struct GlobalMemory *)0)->PotengBar));
                ...
                for (ZBOX=k-1; ZBOX<=k+1; ZBOX++) {
                    ...
                    while (neighbor_ptr) {
                        ...
                        while (curr_ptr) {
                            ...
                            DCACHE_INVAL(&neighbor_ptr->mol.VM[XDIR],sizeof(vm_type));
                            DCACHE_INVAL(&curr_ptr    ->mol.VM[XDIR],sizeof(vm_type));
                            CSHIFT(curr_ptr->mol.F[DISP][XDIR],neighbor_ptr->mol.F[DISP][XDIR],
                                   curr_ptr->mol.VM[XDIR],neighbor_ptr->mol.VM[XDIR],XL,BOXH,BOXL);
                            DCACHE_INVAL(&neighbor_ptr->mol.VM[YDIR],sizeof(vm_type));
                            DCACHE_INVAL(&curr_ptr    ->mol.VM[YDIR],sizeof(vm_type));
                            CSHIFT(curr_ptr->mol.F[DISP][YDIR],neighbor_ptr->mol.F[DISP][YDIR],
                                   curr_ptr->mol.VM[YDIR],neighbor_ptr->mol.VM[YDIR],YL,BOXH,BOXL);
                            DCACHE_INVAL(&neighbor_ptr->mol.VM[ZDIR],sizeof(vm_type));
                            DCACHE_INVAL(&curr_ptr    ->mol.VM[ZDIR],sizeof(vm_type));
    ...
    DCACHE_INVAL(POTA,sizeof(double));
    *POTA = *POTA + LPOTA;
    DCACHE_FLUSH(POTA, sizeof(double));

    DCACHE_INVAL(POTR,sizeof(double));
    *POTR = *POTR + LPOTR;
    DCACHE_FLUSH(POTR, sizeof(double));

    DCACHE_INVAL(PTRF,sizeof(double));
    *PTRF = *PTRF + LPTRF;
    DCACHE_FLUSH(PTRF, sizeof(double));
