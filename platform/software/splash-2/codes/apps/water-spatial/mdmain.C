...
//    double XTT;
    ...
    DCACHE_INVAL(&gl->start,sizeof(((struct GlobalMemory *)0)->start));
    BARRIER(gl->start, NumProcs);
#ifdef SIM_SOCLIB
    ...
    for (i=1;i <= NSTEP; i++) {
        ...
        if (ProcID == 0) {
            ...
            gl->VIR = 0.0;
            DCACHE_FLUSH(&gl->VIR,sizeof(double));
            gl->POTA = 0.0;
            DCACHE_FLUSH(&gl->POTA,sizeof(double));
            gl->POTR = 0.0;
            DCACHE_FLUSH(&gl->POTR,sizeof(double));
            gl->POTRF = 0.0;
            DCACHE_FLUSH(&gl->POTRF,sizeof(double));
            for (dir = XDIR; dir <= ZDIR; dir++){
                gl->SUM[dir] = 0.0;
                DCACHE_FLUSH(&gl->SUM[dir],sizeof(double));
            }
        ...
        if ((ProcID == 0) && (i >= 2)) {
            ...
            DCACHE_FLUSH(&gl->intratime,sizeof(unsigned long));
        }
        DCACHE_INVAL(&gl->SUM, sizeof(double)*3);
        ...
        if ( ((i % NPRINT) == 0) || ((NSAVE > 0) && ((i % NSAVE) == 0))) {
            ...
            if ((ProcID == 0) && (i >= 2)) {
                CLOCK(gl->interend);
                gl->intertime += gl->interend - gl->interstart;
                DCACHE_FLUSH(&gl->intertime, sizeof(unsigned long));
           }

            DCACHE_INVAL(&gl->POTA, sizeof(double));
            POTA=gl->POTA*FPOT;
            DCACHE_INVAL(&gl->POTR, sizeof(double));
            POTR=gl->POTR*FPOT;
            DCACHE_INVAL(&gl->POTRF, sizeof(double));
            POTRF=gl->POTRF*FPOT;
        ...
        if ((ProcID == 0) && (i >= 2)) {
            ...
            DCACHE_FLUSH(&gl->tracktime,sizeof(unsigned long));
       }
