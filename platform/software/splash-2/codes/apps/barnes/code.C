...
string defv[] = {                 /* DEFAULT PARAMETER VALUES              */
    ...
    "nbody=48",                /* number of particles to generate       */
...
//struct GlobalMemory g_GlobalMemory;
//struct CellLockType g_CellLockType;
...
void tab_init()
{
   ...
   for (i = 0; i < NPROC; ++i) {
      ...
      if (Local[i].ctab ==NULL) error("Local[i].ctab ==NULL\n");

      Local[i].ltab = (leafptr) G_MALLOC((maxleaf / NPROC) * sizeof(leaf));
      if (Local[i].ltab ==NULL) error("Local[i].ltab ==NULL\n");
   }
   ...
   Local[0].mybodytab = (bodyptr*) G_MALLOC(NPROC*maxmybody*sizeof(bodyptr));
    if ( Local[0].mybodytab ==NULL) error(" Local[0].mybodytab == NULL\n");
   ...
   if ( Local[0].mycelltab ==NULL) error(" Local[0].mycelltab == NULL\n");
   Local[0].myleaftab = (leafptr*) G_MALLOC(NPROC*maxmyleaf*sizeof(leafptr));
   if ( Local[0].myleaftab ==NULL) error(" Local[0].myleaftab == NULL\n");
  CellLock = (struct CellLockType *) G_MALLOC(sizeof(struct CellLockType));
//   CellLock = &g_CellLockType;
   if ( CellLock == NULL) error(" CellLock == NULL\n");
...
void testdata()
{
   ...
   if (bodytab==NULL) error("bodytab == NULL\n");
